...
PlaybackResult fetch_playback_json(const Loader& loader,
    ...
    // Captured dict + mutex for the response preprocessor. Thread-local, with
    // the same lifetime as pr_buf below, because Apple copies the std::function
    // into the PurchaseRequest and pr_buf is never destructed:
    //   - anything the closure *owns* (e.g. a shared_ptr) is never released,
    //     so it would leak once per call;
    //   - anything on *this frame* would dangle if Apple ever invoked the
    //     preprocessor after run() returned (see the HTTPMessageHolder note in
    //     tokens.cpp for the same hazard on HTTPMessage).
    // A thread_local is valid for as long as the thread that stored it, so both
    // failure modes are gone and the per-call cost is zero.
    struct Capture {
        std::mutex mu;
        void*      cf_dict = nullptr;  // CFDictionaryRef (retained)
        int        hits    = 0;
    };
    static thread_local Capture cap;
    {
        // Reset per call: a previous request on this thread may have left a
        // retained dict behind if Apple double-fired the preprocessor.
        std::lock_guard<std::mutex> g(cap.mu);
        if (cap.cf_dict != nullptr) {
            s.CFRelease(cap.cf_dict);
            cap.cf_dict = nullptr;
        }
        cap.hits = 0;
    }
    ...
    // Balance the preprocessor's CFRetain on every exit path. Previously only
    // the success path released, so the fallback and error paths leaked the
    // whole dispatch plist (m3u8 master included) once per GET /playback.
    struct DictRelease {
        const Symbols& s;
        void*          p;
        ~DictRelease() { if (p != nullptr) s.CFRelease(p); }
    } dict_guard{s, dict};
    ...
    if (dict != nullptr) {
        out.body = cf_to_json(s, dict);  // dict_guard releases after return
        out.ok = true;
        return out;
    }
    ...
    if (purchase_response_obj != nullptr
        ...
        // Release the N shared_ptr elements when the vector dtor is available.
        // Resolved optionally (see Loader::open): if the pinned Apple .so does
        // not export it the slot is null and we keep the old leak instead.
