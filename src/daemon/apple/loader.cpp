...
#undef RESOLVE_FPS
    ...
    // Optional destructor for the playback fallback path (the PurchaseItem
    // vector returned by PurchaseResponse::items). Deliberately resolved
    // outside RESOLVE / fp_ok: a missing dtor must degrade to "skip cleanup"
    // (the old leak behavior), never fail the load or disable /decrypt. Uses
    // its own error string so a miss cannot overwrite the FairPlay diagnostic.
    {
        std::string dtor_err;
        auto resolve_opt = [&](const char* mangled_name, auto* out_slot) {
            void* const handles[] = {
                h_libstoreservicescore_,
                h_libmediaplatform_,
                h_libandroidappmusic_,
            ...
            for (int i = 0; handles[i] != nullptr; ++i) {
                if (resolve(handles[i], mangled_name, out_slot, &dtor_err)) return;
            }
            (void)resolve(RTLD_DEFAULT, mangled_name, out_slot, &dtor_err);
        };
        resolve_opt(mangled::vector_PurchaseItem_dtor,
                    &symbols_.vector_PurchaseItem_dtor);
        if (symbols_.vector_PurchaseItem_dtor == nullptr) {
            std::fprintf(stderr,
                         "loader: optional vector<PurchaseItem> dtor unavailable "
                         "(%s); GET /playback fallback keeps its per-item leak\n",
                         dtor_err.c_str());
        }
    }
