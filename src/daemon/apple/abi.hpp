...
// temporaries passed indirectly). Passing abi::std_string* here
...
//     Used by zhaarey/apple-music-downloader and some APKs.
...
// Optional destructor. Resolved non-fatally (see Loader::open): when the
// pinned Apple .so does not export it, the slot stays null and the caller
// skips the cleanup rather than failing the whole load.
...
//
// 8 string params (= seven `S8_`). Used by zhaarey/apple-music-downloader on
// builds where Apple split adam_id into a separate prefetch slot (adam_id duplicated).
...
// 7 string params (= six `S8_`). What `nm -D` shows on the x86_64 split of
// Apple Music 3.6.0-beta-1109. No prefetch slot — one adam_id, six other
// strings (uri, key format, key format version, server uri, protocol type,
// fps cert).
