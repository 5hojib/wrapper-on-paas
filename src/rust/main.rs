...
const MAX_HTTP_BODY: usize = 512 * 1024 * 1024;

fn main() {
    if let Err(e) = run() {
        eprintln!("wrapper: fatal: {e}");
        std::process::exit(1);
    }
}
...
fn run() -> io::Result<()> {
    ...
    // PaaS (Heroku/Render) inject the public port via $PORT. An explicit
    // WRAPPER_PORT wins so self-hosted/compose config is unaffected.
    let http_port = env_u16_opt("WRAPPER_PORT")
        .or_else(|| env_u16_opt("PORT"))
        .unwrap_or(DEFAULT_HTTP_PORT);
    let decrypt_host = env_or("WRAPPER_DECRYPT_HOST", DEFAULT_DECRYPT_HOST);
    // The raw TCP decrypt listener is optional: single-port PaaS deploys set
    // WRAPPER_DECRYPT_PORT=0 and keep using HTTP POST /decrypt instead.
    // Unset means the upstream default (10020) is used.
    let decrypt_port = env::var("WRAPPER_DECRYPT_PORT")
        .ok()
        .and_then(|v| v.trim().parse::<u16>().ok())
        .map(|v| if v == 0 { None } else { Some(v) })
        .unwrap_or(Some(DEFAULT_DECRYPT_PORT));
    let api_key = env::var("WRAPPER_API_KEY")
        .ok()
        .filter(|s| !s.is_empty());

    let worker = Arc::new(Worker::new("/system/bin/main", VERSION.to_string()));
    worker.ensure_started().map_err(worker_io_error)?;

    if let Some(decrypt_port) = decrypt_port {
        ...
        thread::spawn(move || {
            if let Err(e) = run_decrypt_tcp(&tcp_addr, tcp_worker) {
                eprintln!("wrapper: decrypt tcp listener stopped: {e}");
                std::process::exit(1);
            }
        });
    }
    ...
    run_http(&http_addr, worker, api_key)
}
...
fn run_http(addr: &str, worker: Arc<Worker>, api_key: Option<String>) -> io::Result<()> {
    let listener = TcpListener::bind(addr)?;
    eprintln!("wrapper: {VERSION} HTTP listening on {addr}");
    for conn in listener.incoming() {
        match conn {
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    if let Err(e) = handle_http_connection(stream, worker, api_key) {
                        eprintln!("wrapper: http connection error: {e}");
                    }
                });
            }
            Err(e) => eprintln!("wrapper: http accept error: {e}"),
        }
    }
    Ok(())
}
...
fn handle_http_connection(
    ...
    if let Some(key) = api_key {
        let authorized = req.headers.iter().any(|h| {
            h.name.eq_ignore_ascii_case("authorization")
                && h.value.len() == 7 + key.len()
                && h.value.starts_with(b"Bearer ")
                && h.value[7..].eq_ignore_ascii_case(key.as_bytes())
        });
        if !authorized {
            write_json(
                &mut stream,
                401,
                json!({"error":"unauthorized","detail":"missing or invalid WRAPPER_API_KEY"}),
            )?;
            return Ok(());
        }
    }
    ...
    if content_length > MAX_HTTP_BODY {
        write_json(
            &mut stream,
            413,
            json!({"error":"payload_too_large","limit":MAX_HTTP_BODY}),
        )?;
        return Ok(());
    }
    ...
    match (method.as_str(), path.as_str()) {
        ...
        ("POST", "/decrypt") => {
            let _ = content_type;
            // Same binary protocol the old HTTP supervisor exposed, so
            // single-port PaaS deployments keep working over HTTP:
            //   u32be adam_len, u32be uri_len, u32be sample_count,
            //   u32be sample_len[sample_count], adam, uri, ciphertext samples.
            match protocol::parse_http_decrypt_batch(&body) {
                Ok((adam, uri, samples)) => match worker.decrypt_batch(&adam, &uri, samples) {
                    Ok(plaintexts) => {
                        let payload = protocol::build_http_decrypt_samples_payload(&plaintexts)?;
                        write_response(&mut stream, 200, "application/octet-stream", &payload)?;
                    }
                    Err(e) => write_json(
                        &mut stream,
                        502,
                        json!({"error":"decrypt_failed","detail":e.to_string()}),
                    )?,
                },
                Err(e) => write_json(
                    &mut stream,
                    400,
                    json!({"error":"invalid_frame","detail":e.to_string()}),
                )?,
            }
        }
...
fn run_decrypt_tcp(addr: &str, worker: Arc<Worker>) -> io::Result<()> {
    let listener = TcpListener::bind(addr)?;
    eprintln!("wrapper: {VERSION} TCP decrypt listening on {addr}");
    for conn in listener.incoming() {
        match conn {
            Ok(stream) => {
                let worker = Arc::clone(&worker);
                thread::spawn(move || {
                    if let Err(e) = handle_decrypt_client(stream, worker) {
                        eprintln!("wrapper: decrypt client closed: {e}");
                    }
                });
            }
            Err(e) => eprintln!("wrapper: decrypt accept error: {e}"),
        }
    }
    Ok(())
}
...
fn handle_decrypt_client(mut stream: TcpStream, worker: Arc<Worker>) -> io::Result<()> {
    ...
    loop {
        ...
        if !logged_session {
            eprintln!(
                "wrapper: decrypt client {peer} adam={} fps_key_uri={}",
                adam, uri
            );
            logged_session = true;
        }
